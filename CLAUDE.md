...
- VM 102 compute: `ssh austin@192.168.1.105` (sudo for systemctl). LXC 120 StoneSage: `ssh root@192.168.1.167`.
  LXC 128 Frigate: `ssh root@192.168.1.150`. pve node: `ssh root@192.168.1.222` (key added 2026-09-24; shared cluster-wide).
- Proxmox: API at `https://192.168.1.245:8006` (bigserv; per `/cluster/status`: pve = 192.168.1.222, bigserv = .245;
  HA OS VM = .82). Prefer the API for guest config; SSH to the nodes for device passthrough and `pct`/`qm`.
