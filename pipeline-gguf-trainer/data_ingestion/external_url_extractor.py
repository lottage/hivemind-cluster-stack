...
class ExternalUrlExtractor:
    def __init__(
        self,
        cluster_worker_url: str = "http://192.168.1.105:8002/v1",
        ...
        if parsed.scheme == "obsidian":
            ...
            try:
                ...
                if doc_name:
                    ...
                    couch_base = os.environ.get("COUCHDB_URL", "http://192.168.1.230:5984")
                    couch_url = f"{couch_base}/obsidiannotes/{doc_id}"
                    req = urllib.request.Request(couch_url)
                    _cu, _cp = os.environ.get("COUCHDB_USER", ""), os.environ.get("COUCHDB_PASSWORD", "")
                    if _cu:
                        import base64 as _b64
                        req.add_header("Authorization", "Basic " + _b64.b64encode(f"{_cu}:{_cp}".encode()).decode())
