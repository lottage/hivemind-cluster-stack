...
> **Sanitized GitHub Export**: [`c:\Users\johna\OneDrive\Documents\.ai\hivemind-cluster-stack`](file:///c:/Users/johna/OneDrive/Documents/.ai/hivemind-cluster-stack)  
...
| :--- | :--- | :--- | :--- | :--- |
| **Proxmox Cluster VIP** | `https://192.168.1.245` | `:8006` | Central Datacenter PVE 9.2 API daemon managing `pve` & `bigserv` | Header: `PVEAPIToken=root@pam!StoneSage=***REMOVED-PVE-TOKEN***` |
| **Node 1: `pve`** | `192.168.1.229` | `:8006` | Physical Host 1: Intel i7-12700K, 32GB RAM, Dual AMD GPUs | SSH: `austin@192.168.1.229` |
| **VM 102: `ubu`** | `192.168.1.105` | `:8001`<br>`:8002`<br>`:8003`<br>`:6379`<br>`:8765`<br>`:8766` | **Dual-GPU Compute Host (Ubuntu 24.04)**:<br>• `:8001` Coordinator: `Ornith-1.5-9B-Instruct` (Q8_0) on RX 6750 XT 12GB (`Vulkan0`)<br>• `:8002` Worker: `Ornith-1.5-9B-Instruct` (Q4_K_M) on RX 6600 XT 8GB (`Vulkan1`)<br>• `:8003` Embedder: `bge-large-en-v1.5` (F16) on RX 6600 XT 8GB (`Vulkan1`)<br>• `:6379` **Valkey 9.0.4 In-RAM Store**: Sub-ms A-MEM atomic working memory & tag index<br>• `:8765` **Cluster MCP Bridge & Autonomous Engine**: Starlette SSE/JSON-RPC daemon<br>• `:8766` **Sovereign Agent Assembly Hall**: Multi-channel real-time agent streaming fabric | SSH: `austin@192.168.1.105`<br>Local model endpoints use standard OpenAI-compatible API (`/v1/chat/completions`) |
...
| **LXC 116: `obsidian-live-sync`**| `192.168.1.230` | `:5984` | Apache CouchDB (Obsidian LiveSync server for encrypted notes) | User: `austin` / Pass: `***REMOVED-COUCHDB-PASSWORD***`<br>Vault E2EE Passphrase: `***REMOVED-COUCHDB-PASSWORD***` |
...
- **StoneSage Web Configuration**: [`StoneSage/backend/config.json`](file:///c:/Users/johna/OneDrive/Documents/.ai/StoneSage/backend/config.json) — IPs, tokens, and model routing.
- **StoneSage Web Frontend**: [`StoneSage/frontend/index.html`](file:///c:/Users/johna/OneDrive/Documents/.ai/StoneSage/frontend/index.html) & [`app.js`](file:///c:/Users/johna/OneDrive/Documents/.ai/StoneSage/frontend/app.js) — Windows 95/98 retro pre-bloat HUD.
- **StoneSage WebSocket Broker**: [`server setup/stonesage-ws/ws_broker.py`](file:///c:/Users/johna/OneDrive/Documents/.ai/server%20setup/stonesage-ws/ws_broker.py) — Duplex streaming & live poller (Port 8086).
- **StoneSage Mobile App (React Native / Expo)**: [`stonesage-client/App.tsx`](file:///c:/Users/johna/OneDrive/Documents/.ai/stonesage-client/App.tsx) — Full portable mobile client with Server Connection Studio, HF Model Browser, Harness Selector, Task Routing Matrix, and Rumination HUD.
- **Installed Android APK**: Hosted on LXC 120 at `http://192.168.1.167:8080/stonesage.apk` (and locally at [`StoneSage/frontend/stonesage.apk`](file:///c:/Users/johna/OneDrive/Documents/.ai/StoneSage/frontend/stonesage.apk)).
...
- **Clean Open-Source Repository**: [`c:\Users\johna\OneDrive\Documents\.ai\hivemind-cluster-stack`](file:///c:/Users/johna/OneDrive/Documents/.ai/hivemind-cluster-stack) — 100% sanitized of all private IPs, passwords, and personal identifiers.
...
- Obsidian LiveSync uses End-to-End Encryption (AES-256-GCM with passphrase `***REMOVED-COUCHDB-PASSWORD***`).
...
5. **Proxmox VE API Token Format & Privilege Separation**:
   ...
   - In Proxmox, the token secret **is** a Version-4 UUID (e.g. `root@pam!StoneSage=***REMOVED-PVE-TOKEN***`).
