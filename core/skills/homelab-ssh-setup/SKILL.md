...
type "$HOME\.ssh\id_ed25519.pub" | ssh clusteradmin@127.0.0.1 "mkdir -p ~/.ssh && chmod 700 ~/.ssh && cat >> ~/.ssh/authorized_keys && chmod 600 ~/.ssh/authorized_keys"
```
...
```powershell
ssh -o BatchMode=yes clusteradmin@127.0.0.1 "echo '[SUCCESS] Passwordless SSH is active!'"
```
