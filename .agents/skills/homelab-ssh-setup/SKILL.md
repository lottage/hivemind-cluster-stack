...
description: >-
  ...
  to Linux homelab hosts, Proxmox VE nodes, and VMs (e.g. Ubuntu VM 192.168.1.105).
...
type "$HOME\.ssh\id_ed25519.pub" | ssh austin@192.168.1.105 "mkdir -p ~/.ssh && chmod 700 ~/.ssh && cat >> ~/.ssh/authorized_keys && chmod 600 ~/.ssh/authorized_keys"
```
...
```powershell
ssh -o BatchMode=yes austin@192.168.1.105 "echo '[SUCCESS] Passwordless SSH is active!'"
```
