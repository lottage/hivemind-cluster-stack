...
def retrieve_rag_context(query: str, max_chunks: int = 3) -> str:
    """
    Queries Qdrant vector memory across Austin's Obsidian vault, companion profile,
    ...
    try:
            ...
            with urllib.request.urlopen(req, timeout=4) as resp:
                ...
                for pt in data.get("result", []):
                    if pt.get("score", 0) >= 0.65:
                        ...
                        retrieved_snippets.append(f"--- [Austin's Record: {title}] ---\n{content.strip()[:400]}")
            ...
            with urllib.request.urlopen(req, timeout=4) as resp:
                ...
                for pt in data.get("result", []):
                    if pt.get("score", 0) >= 0.65:
                        ...
                        content = p.get("content") or p.get("text", "")
                        retrieved_snippets.append(f"--- [Austin's Vault Note: {title}] ---\n{content.strip()[:400]}")
        except Exception as e:
        ...
        if retrieved_snippets:
            return (
                "\n\n### Grounded Records from Austin's Obsidian Knowledge Vault (For Reference Only):\n"
                "[IMPORTANT: The following are Austin's personal human records and projects. "
