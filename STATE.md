...
- PTZ patrol (2026-09-25, `backend/patrol.py`, `config.json patrol`, LIVE tab line "🛡️ Patrol" + "Sweep now"):
  ...
  steps right with a frame each until the picture stops changing (right end stop) or `max_frames`, then back to where
  it pointed before the sweep (2026-09-25: saved as a temporary Tapo preset "StoneSage return" via
  `tapo_control.save_preset`, deleted after; HA's preset list refreshes on a save but not on a delete, so the name can
  linger there and camera_ui / camera_scan hide it). Home preset (kitchen "Living Room", driveway "Doors") only when the
  save or return fails.
...
- Presence corrections (2026-09-25, `backend/presence_corrections.py`, cards in Residents & Pets): "✗ Wrong" and
  ...
  Presence refreshes wait while a card is mid-correction.
  Subject boxes (2026-09-25, `backend/sighting_boxes.py`, `GET /api/presence/box?source=&ref=&name=`): a 2px gold box on
  each card's picture around the subject the label means, drawn by the browser over the image (which shifts to keep
  the subject in the 90 px strip). Frigate: the event's own `data.box`. Sentry/patrol: Qwen2.5-VL grounding ("Locate the
  dog (long-haired dachshund...)"), 1-3 s once per sighting, cached in `data/sighting_boxes.json` (misses retried after
  1 h). A person correction from a sentry/patrol picture now crops to that person (grounded by the NEW name) before
  the upload to Frigate's face library, so the right face is learned when several people are in view. Next-most-recent per source: Frigate keeps 10 sightings per
